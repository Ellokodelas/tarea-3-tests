...
func (t *PeerTable) Update(key PeerKey, inventory []Item, vetos []VetoEntry) {
	...
	copy(vetoCp, vetos)
	snap := PeerSnapshot{Inventory: invCp, Vetos: vetoCp}
...
func (t *PeerTable) loadFromDisk() {
	// Buscar archivos que coincidan con el patrón de este owner.
	...
	for _, path := range matches {
		var key PeerKey
		// Extraer MachineID y ProcessID del nombre del archivo.
		// Formato: logs/peer_M<oM>P<oP>_copia_M<m>P<p>.json
		_, err := fmt.Sscanf(
			filepath.Base(path),
